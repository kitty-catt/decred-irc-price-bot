...
exports.Server = function(serverSettings)
{
	...
	this.connect = function() {
		...
		this.socket.on("connect", function() { 
			...
			if (typeof self.pass != 'undefined') {
				self.sendCommand("PASS", self.pass);
			}
		...
		switch(message.command)
		{
			...
			case "251":
				...
				var joinchans = function() {
					self.sendCommand("PRIVMSG", settings.connectcmd);
					setTimeout(function() {
						for(var channel in self.channels)
						{
							self.sendCommand("JOIN", channel);
							self.channels[channel].modules.start();
						}
					}, 5000);			
				};
